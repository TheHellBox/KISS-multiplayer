...
local function get_current_time()
    ...
    date.sec = 0
    date.min = 0
    return (network.socket.gettime() + network.connection.time_offset -
               os.time(date))
end

local function enable_spawning(enabled)
    local jsCommand =
        'angular.element(document.body).injector().get("VehicleSelectConfig").configs.default.hide = {"spawnNew":' ..
            tostring(not enabled) .. '}'
    be:executeJS(jsCommand)
end

local function color_eq(a, b)
    return (a[1] == b[1]) and (a[2] == b[2]) and {a[3] == b[3]} and
               {a[4] == b[4]}
end

local function colors_eq(a, b)
    return color_eq(a[1], b[1]) and color_eq(a[2], b[2]) and
               color_eq(a[3], b[3])
end
...
local function send_vehicle_update(obj)
    ...
    network.send_data({VehicleUpdate = result}, false)
end
...
local function send_vehicle_meta_updates()
    for i = 0, be:getObjectCount() do
        ...
        if vehicle then
            ...
            if changed then
                local data = {VehicleMetaUpdate = {id, plate, colors}}
...
local function update_ownership_limits()
    ...
    if owned_vehicle_count >=
        network.connection.server_info.max_vehicles_per_client then
...
local function send_vehicle_config_inner(id, parts_config, data)
    ...
    network.send_data({VehicleData = vehicle_data}, true)
end
...
local function spawn_vehicle(data)
    ...
    if kisstransform.raw_transforms[data.server_id] then
        away =
            (vec3(kisstransform.raw_transforms[data.server_id].position):distance(
                vec3(getCameraPosition())) > kissui.view_distance[0])
    else
        away = (vec3(data.position):distance(vec3(getCameraPosition())) >
                   kissui.view_distance[0])
    end
    ...
    if data.owner == network.get_client_id() then
        ...
        be:getObjectByID(data.in_game_id):queueLuaCommand(
            "extensions.hook('kissUpdateOwnership', true)")
        return
    end
    ...
    local options = {
        vehicleName = "mp_veh",
        paint = createVehiclePaint({x = c[1], y = c[2], z = c[3], w = c[4]},
                                   metal_data),
        paint2 = createVehiclePaint({
            x = cp0[1],
            y = cp0[2],
            z = cp0[3],
            w = cp0[4]
        }, metal_data),
        paint3 = createVehiclePaint({
            x = cp1[1],
            y = cp1[2],
            z = cp1[3],
            w = cp1[4]
    ...
    local spawned = spawn.spawnVehicle(name, serialize(parts_config),
                                       vec3(data.position), quat(data.rotation),
                                       options)
...
local function onUpdate(dt)
    ...
    if (getMissionFilename():lower() ~=
        network.connection.server_info.map:lower()) and
        (getMissionPath():lower() ~= network.connection.server_info.map:lower()) and
        not M.loading_map then network.disconnect() end
    ...
    if not (M.loading_map or M.delay_spawns) then
        ...
        for k, vehicle in pairs(M.vehicle_buffer) do
            ...
            if t and
                not ((vec3(t.position):distance(vec3(getCameraPosition())) >
                    kissui.view_distance[0]) and kissui.enable_view_distance[0]) then
                spawn_vehicle(vehicle)
                table.insert(to_remove, k)
            end
        end
        for _, v in pairs(to_remove) do M.vehicle_buffer[v] = nil end
    end
end
...
local function update_vehicle(data)
    ...
    if character then
        kissplayers.player_transforms[data.vehicle_id].target_position = vec3(
                                                                             data.transform
                                                                                 .position)
        kissplayers.player_transforms[data.vehicle_id].rotation = data.transform
                                                                      .rotation
        kissplayers.player_transforms[data.vehicle_id].velocity = vec3(
                                                                      data.transform
                                                                          .velocity)
        kissplayers.player_transforms[data.vehicle_id].time_past = clamp(
                                                                       get_current_time() -
                                                                           data.sent_at,
                                                                       0, 0.3) +
                                                                       0.0001
    ...
    if not kisstransform.inactive[id] then
        vehicle:queueLuaCommand("kiss_input.apply(\'" ..
                                    jsonEncode(data.electrics) .. "\')")
        vehicle:queueLuaCommand("kiss_gearbox.apply(\'" ..
                                    jsonEncode(data.gearbox) .. "\')")
    end
end
...
local function reset_vehicle(data)
    ...
    if vehicle then
        ...
        if kisstransform.local_transforms[id] then
            ...
            vehicle:setPositionRotation(position.x, position.y, position.z,
...
local function update_vehicle_meta(data)
    ...
    local plate = data.plate
    local colors = {
        data.colors_table[1], data.colors_table[2], data.colors_table[3]
    }

    -- Apply plate
    if plate ~= nil then extensions.core_vehicles.setPlateText(plate, id) end
...
local function attach_coupler_inner(data)
    ...
    network.send_data({CouplerAttached = data}, true)
end
...
local function detach_coupler_inner(data)
    ...
    data.obj_a = M.server_ids[data.obj_a]
    data.obj_b = M.server_ids[data.obj_b]
    network.send_data({CouplerDetached = data}, true)
end
...
local function attach_coupler(data)
    ...
    if obj_a and obj_b then
        ...
        if vec3(vehicle:getPosition()):distance(vec3(vehicle_b:getPosition())) >
            15 then return end
        local node_a_pos = vec3(vehicle:getPosition()) +
                               vec3(vehicle:getNodePosition(data.node_a_id))
        local node_b_pos = vec3(vehicle_b:getPosition()) +
                               vec3(vehicle_b:getNodePosition(data.node_b_id))
        local pos = vec3(vehicle_b:getPosition()) + (node_a_pos - node_b_pos)
        vehicle_b:setPosition(Point3F(pos.x, pos.y, pos.z))
        vehicle_b:queueLuaCommand("kiss_couplers.attach_coupler(" ..
                                      data.node_b_id .. ")")
...
local function detach_coupler(data)
    ...
    if obj_a and obj_b then
        ...
        if vec3(vehicle:getPosition()):distance(vec3(vehicle_b:getPosition())) >
            15 then return end
        vehicle:queueLuaCommand("kiss_couplers.detach_coupler(" ..
                                    data.node_a_id .. ")")
...
local function onVehicleSpawned(id)
    ...
    if first_vehicle then
        vehicle:setPosition(Point3F(position.x + math.random(-5, 5),
                                    position.y + math.random(-5, 5), position.z))
    ...
    if vehicle:getJBeamFilename() == "unicycle" then
        for i = 0, be:getObjectCount() do
            ...
            if v and (v:getID() ~= vehicle:getID()) and
                (v:getJBeamFilename() == "unicycle") then v:delete() end
        end
    end
end
...
local function onVehicleDestroyed(id)
    ...
    if M.ownership[id] then
        ...
        M.ownership[id] = nil
        network.send_data({RemoveVehicle = id}, true)
        update_ownership_limits()
    end
end
...
local function onVehicleResetted(id)
    ...
    if M.ownership[id] then network.send_data({ResetVehicle = id}, true) end
end

local function onVehicleSwitched(old_id, new_id)
    local oldVehicle = be:getObjectByID(old_id)
    local newVehicle = be:getObjectByID(new_id)
    ...
    if oldName == "unicycle" and newName ~= oldName then
        oldVehicle:delete()
    end

    if M.ownership[new_id] then
        log('D', 'kissmp', 'new ownership')
        network.send_data({VehicleChanged = new_id}, true)
    end
end
