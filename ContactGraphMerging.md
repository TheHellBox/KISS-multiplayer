...
This document sits on top of `NodeClusterSync.md`. That document covers
intra-vehicle cluster decomposition, per-cluster pose sync, and the
damage sync pipeline. **This document supersedes** the inter-vehicle
parts of `NodeClusterSync.md` — specifically §3.3 (server-side passive
coupling graph), the cross-vehicle tail of §4.2, and the old Phase 5
"tractor+trailer different owners" migration path. Those sections are
replaced by the contact graph + authority election machinery described
below.

What stays in force from `NodeClusterSync.md`:
- §3.1–§3.2, §4.1, §4.3–§4.6 — intra-vehicle discovery, topology,
  frames, per-cluster pose computation, runtime re-cluster.
- All damage sync channels (Phase 3.5 beam break events, Phase 10
  per-node deformation). Those layer on top of whatever pose channel
  the contact graph routes state through.
